...
public final class IntBitSet extends AbstractIntSet implements Cloneable {
    ...
    public IntBitSet(int capacity, long[] data) {
        if (capacity > (data.length << 6))
            throw new IllegalArgumentException("capacity too large: " + capacity + ", max: " + (data.length << 6));
        this.capacity = capacity;
        this.elements = data;
        recalculateSize();
