...
public final class IdentityHashSet<T> extends AbstractSet<T> {
    ...
    private final class IdentityIterator implements Iterator<T> {
        private int index = (size != 0 ? 0 : table.length); // current slot.
        private int lastReturnedIndex = -1;      // to allow remove()
        private Object[] traversalTable = table; // reference to main table or copy
