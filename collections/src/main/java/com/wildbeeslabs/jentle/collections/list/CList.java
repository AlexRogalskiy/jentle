...
public class CList<T> implements IList<T> {
    ...
    public void deleteDuplicates(final CListNode<T> node) {
        Set<T> set = new HashSet<>();
        CListNode<T> previous = node, current = node;
    ...
    public void deleteDuplicates2(final CListNode<T> node) {
        CListNode<T> current = node;
        while (Objects.nonNull(current)) {
            CListNode<T> runner = current;
    ...
    public T getKthToLast(final CListNode<T> node, int k) {
        Integer idx = new Integer(0);
        CListNode<T> current = this.getKthToLast(node, k, idx);
        return Objects.nonNull(current) ? current.data : null;
    }

    private CListNode<T> getKthToLast(final CListNode<T> node, int k, Integer idx) {
        if (this.isEmpty()) {
            return null;
        }
        CListNode<T> current = getKthToLast(node.next, k, idx);
        idx = idx + 1;
        if (idx.intValue() == k) {
            return node;
        }
    ...
    public T getKthToLast2(final CListNode<T> node, int k) {
        CListNode<T> p1 = node, p2 = node;
        for (int i = 0; i < k; i++) {
