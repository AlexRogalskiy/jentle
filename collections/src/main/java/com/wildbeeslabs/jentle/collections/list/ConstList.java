...
public final class ConstList<T> extends AbstractList<T> implements Serializable, RandomAccess {
    ...
    public static final class TempList<T> {
         ...
         */
        private void chk() {
            if (clist != null) throw new UnsupportedOperationException();
        }
        ...
        public TempList<T> addAll(Iterable<? extends T> all) {
            chk();
            if (all instanceof Collection) list.addAll((Collection<? extends T>) all);
            else if (all != null) {
                for (T x : all) list.add(x);
            }
            return this;
        }
        ...
        public ConstList<T> makeConst() {
            if (clist == null) clist = (list.isEmpty() ? emptylist : new ConstList<T>(list));
            return clist;
        }
    }
    ...
    public static <T> ConstList<T> make(Iterable<T> collection) {
        ...
        if (collection instanceof Collection) {
            ...
            if (col.isEmpty()) return make();
            else return new ConstList<T>(new ArrayList<T>(col));
        }
        ...
        else return new ConstList<T>(ans);
    }
