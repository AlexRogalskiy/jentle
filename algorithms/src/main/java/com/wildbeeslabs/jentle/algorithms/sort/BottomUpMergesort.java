...
import static com.wildbeeslabs.jentle.algorithms.sort.MergesAndRuns.mergeRuns;
...
//        minRunLen = myMinRunLen;
    ...
    private static final int minRunLen = 24;
    ...
    public static void mergesortCheckSorted(int[] A, int left, int right, int[] B) {
        int n = right - left + 1;
        if (minRunLen != 1) {
            for (int len = minRunLen, i = left; i <= right; i += len) {
                Insertionsort.insertionsort(A, i, min(i + len - 1, right));
