...
public class H {
    ...
    public static double calculationByDistance(final GeoPosition StartP, final GeoPosition EndP) {
    ...
    public static float stringToFloat(String s) {
        if (s == null || s.equals("")) {
            CValidationUtils.notEmpty("H", "Empty string instead of float");
            return .0f;
        }
        try {
            return Float.parseFloat(s);
        } catch (NumberFormatException e) {
            CValidationUtils.notEmpty("stringToFloat", "Fallback to Numberformat");
            try {
