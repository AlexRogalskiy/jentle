...
public class MergesAndRuns {
    ...
    public static final boolean COUNT_MERGE_COSTS = false;
    /** total merge costs of all merge calls */
    public static long totalMergeCosts = 0;

    public static final boolean COUNT_MOVE_COSTS = false;
    public static long totalMoveCosts = 0;
     ...
     */
    public static void mergeRuns(final int[] A, final int left, int m, final int right, final int[] buffer) {
        if (MergesAndRuns.COUNT_MOVE_COSTS) {
            MergesAndRuns.totalMoveCosts += (right - left) * 2 + 1;
        }
        if (true) {
            // Merge sorted halves [l..r] both inclusive
            System.arraycopy(A, left, buffer, left, right - left + 1);

            for (int i = left, p = left, q = m; i <= right; i++) {
                if ((q > right) || ((p < m) && (buffer[p] <= buffer[q]))) {
                    A[i] = buffer[p++];
                } else {
                    A[i] = buffer[q++];
                }
            ...
            for (j = m; j < right; ++j) {
                buffer[right + m - j] = A[j + 1];
            }
            for (int k = left; k <= right; ++k) {
                A[k] = buffer[j] < buffer[i] ? buffer[j--] : buffer[i++];
            }
        }
    }
