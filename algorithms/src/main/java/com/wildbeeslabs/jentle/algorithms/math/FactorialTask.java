...
public class FactorialTask extends RecursiveTask<BigInteger> {
    ...
    protected BigInteger compute() {
        if ((n - start) >= THRESHOLD) {
            ...
            return calculate(this.start, this.n);
        }
    }

    private Collection<FactorialTask> createSubtasks() {
        final List<FactorialTask> dividedTasks = new ArrayList<>();

        int mid = (this.start + this.n) / 2;

        dividedTasks.add(new FactorialTask(this.start, mid));
        dividedTasks.add(new FactorialTask(mid + 1, this.n));
        return dividedTasks;
    }
