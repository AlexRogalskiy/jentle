...
public class CalendarParser {
     ...
     * @return parsed date
     * @throws IllegalArgumentException if there was a problem parsing the string.
     */
     ...
     * @param order         order in which pieces of numeric strings are assigned (should
     *                      be one of <tt>YY_MM_DD</tt>, <tt>MM_DD_YY</tt>, etc.)
     * @param ignoreChanges if <tt>true</tt>, ignore date changes such as <tt>Feb 31</tt>
     *                      being changed to <tt>Mar 3</tt>.
     * @return parsed date
     * @throws IllegalArgumentException if there was a problem parsing the string.
     */
    public static final Calendar parse(String dateStr, int order,
                                       boolean ignoreChanges) throws IllegalArgumentException {
        if (dateStr == null) {
            return null;
        }
     ...
     * @throws IllegalArgumentException if there was a problem parsing the token
     */
    private static final void parseNonNumericToken(String dateStr,
                                                   ParserState state, String token) throws IllegalArgumentException {
        ...
        if (token.indexOf(':') > 0) {
            ...
            if (Character.isDigit(firstChar)) {
                ...
                throw new IllegalArgumentException("Unrecognized time \""
        ...
        if (tmpMon != UNSET) {
            ...
            throw new IllegalArgumentException("Too many numbers in"
                + " date \"" + dateStr + "\"");
        }
        ...
        if (val == UNSET) {
            ...
            throw new IllegalArgumentException("Unknown string \"" + token
                + "\" in date \"" + dateStr + "\"");
        }
        ...
        throw new IllegalArgumentException("Cannot assign ordinal in \""
            + dateStr + "\"");
    }
     ...
     * @throws IllegalArgumentException if there was a problem splitting the value
     */
    private static final void parseNumericBlob(String dateStr,
                                               ParserState state, int val) throws IllegalArgumentException {
    ...
    private static final void parseNumericToken(String dateStr,
        ...
        if (state.isYearSet() && state.isMonthSet() && state.isDateSet()) {
            ...
            throw new IllegalArgumentException("Extra value \"" + val
                + "\" in date \"" + dateStr + "\"");
        }
        ...
        if (val < 0) {
            ...
            throw new IllegalArgumentException("Found negative number in"
                + " date \"" + dateStr + "\"");
        }
        ...
        if (val > 31) {
            ...
            if (state.getYear() > 12) {
                ...
                throw new IllegalArgumentException("Bad number " + val
     ...
     *                      being changed to <tt>Mar 3</tt>.
     * @return parsed date
     * @throws IllegalArgumentException if no valid date was found.
     */
    private static final Calendar parseString(String dateStr, int order,
        ...
        if (!state.isDateSet()) {
            if (!state.isMonthSet()) {
                if (!state.isYearSet()) {
                    throw new IllegalArgumentException("No date found in \""
                        + dateStr + "\"");
                } else {
                    throw new IllegalArgumentException("Day and month missing"
                        + " from \"" + dateStr + "\"");
                }
            } else {
                throw new IllegalArgumentException("Day missing from \""
                    + dateStr + "\"");
            }
        } else if (!state.isMonthSet()) {
            if (!state.isYearSet()) {
                throw new IllegalArgumentException("Year and month missing"
                    + " from \"" + dateStr + "\"");
            } else {
                throw new IllegalArgumentException("Month missing from \""
                    + dateStr + "\"");
            }
        } else if (!state.isYearSet()) {
            throw new IllegalArgumentException("Year missing from \"" + dateStr
                + "\"");
        }
     ...
     * @throws IllegalArgumentException if there is a problem with the time
     */
    private static final void parseTime(String dateStr, ParserState state,
                                        String timeStr) throws IllegalArgumentException {
            ...
            switch (place) {
                case PLACE_HOUR:
                    try {
                        ...
                        throw new IllegalArgumentException(dfe.getMessage()
                            + " in \"" + dateStr + "\"");
                    }
                    if (DEBUG) {
                ...
                case PLACE_MILLI:
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("Unexpected place value "
                        + place);
            }
        }
    }
    ...
    private static final void parseTimeZoneOffset(String dateStr,
                                                  ParserState state, String zoneStr) throws IllegalArgumentException {
        int place = PLACE_HOUR;

        final boolean isNegative = (zoneStr.charAt(0) == '-');
        if (!isNegative && zoneStr.charAt(0) != '+') {
            throw new IllegalArgumentException("Bad time zone offset \""
        ...
        for (String token : tList) {
            ...
            try {
                ...
                throw new IllegalArgumentException("Bad time zone "
            ...
            switch (place) {
                ...
                case PLACE_MINUTE:
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("Unexpected place value " + place);
            }
        }
