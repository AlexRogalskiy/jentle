...
public class CUtils {
    ...
    public static <T> T newInstance(final Class<T> c) {
        if (c == null)
            throw new ResourceNotFoundException("class cannot be null");
        try {
            return c.getDeclaredConstructor().newInstance();
        } catch (NoSuchMethodException e) {
            throw new ResourceNotFoundException("Could not find a public no-argument constructor for " + c.getName(), e);
        } catch (ReflectiveOperationException | RuntimeException e) {
            throw new ResourceNotFoundException("Could not instantiate class " + c.getName(), e);
        }
    }
    ...
    public static <T> T newInstance(String klass, Class<T> base) throws ClassNotFoundException {
        return CUtils.newInstance(loadClass(klass, base));
    }
    ...
    public static <T> Class<? extends T> loadClass(String klass, Class<T> base) throws ClassNotFoundException {
        return Class.forName(klass, true, CUtils.getContextOrKafkaClassLoader()).asSubclass(base);
    }
    ...
    public static <T> T newParameterizedInstance(String className, Object... params)
        ...
        try {
            Class<?> c = Class.forName(className, true, CUtils.getContextOrKafkaClassLoader());
            ...
            throw new ClassNotFoundException(String.format("Failed to find " +
                "constructor with %s for %s", CUtils.join(argTypes, ", "), className), e);
    ...
    public static String readFileAsString(String path) throws IOException {
        return CUtils.readFileAsString(path, Charset.defaultCharset());
    }
    ...
    /**
     * Creates a map entry (for use with {@link CUtils#mkMap(java.util.Map.Entry[])})
     *
     * @param k   The key
     * @param v   The value
    ...
    public static ClassLoader getKafkaClassLoader() {
        return CUtils.class.getClassLoader();
    }
